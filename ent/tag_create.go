...
func (tc *TagCreate) AddRanobeIDs(ids ...ulid.ID) *TagCreate {
	tc.mutation.AddRanobeIDs(ids...)
	return tc
}

// AddRanobes adds the "ranobes" edges to the Ranobe entity.
func (tc *TagCreate) AddRanobes(r ...*Ranobe) *TagCreate {
	ids := make([]ulid.ID, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return tc.AddRanobeIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.RanobesIDs(); len(nodes) > 0 {
