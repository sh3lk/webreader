...
func (tq *TagQuery) QueryRanobes() *RanobeQuery {
	query := &RanobeQuery{config: tq.config}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withRanobes: tq.withRanobes.Clone(),
...
func (tq *TagQuery) WithRanobes(opts ...func(*RanobeQuery)) *TagQuery {
	query := &RanobeQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withRanobes = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withRanobes; query != nil {
		if err := tq.loadRanobes(ctx, query, nodes,
			func(n *Tag) { n.Edges.Ranobes = []*Ranobe{} },
			func(n *Tag, e *Ranobe) { n.Edges.Ranobes = append(n.Edges.Ranobes, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedRanobes {
		if err := tq.loadRanobes(ctx, query, nodes,
			func(n *Tag) { n.appendNamedRanobes(name) },
			func(n *Tag, e *Ranobe) { n.appendNamedRanobes(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TagQuery) loadRanobes(ctx context.Context, query *RanobeQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Ranobe)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[ulid.ID]*Tag)
	nids := make(map[ulid.ID]map[*Tag]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.RanobesTable)
		s.Join(joinT).On(s.C(ranobe.FieldID), joinT.C(tag.RanobesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.RanobesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.RanobesPrimaryKey[1]))
...
func (tq *TagQuery) WithNamedRanobes(name string, opts ...func(*RanobeQuery)) *TagQuery {
	query := &RanobeQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedRanobes == nil {
		tq.withNamedRanobes = make(map[string]*RanobeQuery)
	}
	tq.withNamedRanobes[name] = query
	return tq
}
